...
for(let i = 0; i < v_intervale.length; i++){
    for(let j = v_intervale[i][0]; j <= v_intervale[i][1]; j++){ //48-56
        sirAlphaNum+= String.fromCharCode(j); //from char code - coduri ascii transforma in strng si le concatenez la sirAlphaNum
    }
}
...
function genereazaToken(n){
    ...
    //0-1 * sir =  lungimea sirului si il rotunjim cu FLOOR
